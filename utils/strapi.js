...
export async function getPosts(page) {
  ...
  const postsRes = await fetch(API_BASE_URL + `blogs_dubai_collection_?fields=id,title,image,content,date_created,slug,blog_categories.*.*.*,author.*.*.*&page=${page}&sort=sort,-date_created&limit=9&meta=*`);
  const posts = await postsRes.json();
  return posts;
}
